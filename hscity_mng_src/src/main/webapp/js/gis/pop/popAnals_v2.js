...
function popAnalsPeriod(id, mapType){
	...
	if(chkTsAnals(mapType) && chkMultiAreaCnt(26)){ //common.js 시계열 분석 체크 및 선택 동 수 체크
		...
		mapObj.setVisibleLayer("flowLayer", false);
		mapObj.setVisibleLayer("geoserverLayer", false);
		// mapObj.clearRemoveLayer("popLayer"+id); //wms layer 초기화

		getSelectAreas(mapType, function() {
			var features = mapObj.getFeatures("analysisLayer");
			if( features.length == 0 ) {
				openInfoPopup(true,"분석영역이 없습니다.");
				return;
			}
			 ...
			 $.ajax({
				...
				success:function(data){
					...
					if(data.result != null && data.result.length > 1){
						...
						//엑셀 조회용 params저장
						console.log( gridId, popParams, lndParams );
...
//인구분석 wms layer 추가
function addAnalsPopWmsLayer(datas){
	console.log("addAnalsPopWmsLayer",datas);
	var mapObj = gis;
	mapObj.addAnalsPopWmsLayer(datas);
}
//유동인구분석 wms layer 추가
function addAnalsfPopWmsLayer(datas){
	console.log("addAnalsfPopWmsLayer",datas);
	var mapObj = gis;
	mapObj.addAnalsfPopWmsLayer(datas);
}
...
function getAnalsWMSParams(mapObj, id){
	 ...
	 var wmsEmember = '';
	 if(id == '1-1-1') {
		 wmsUrl = "hsp_rsgst_popltn";
		 wmsYear = $('#txtBasicDate').val().split('-')[0];
		 wmsGender = ["m", "w"];
		 wmsSage = "0";
		 wmsEage = "999";
		 wmsSmember = "0";
		 ...
		 wmsYear = $('#txtBasicDate').val().split('-')[0];
		 if(chkYear == 'cur'){
			 wmsYear =  new Date().getFullYear();
		 }
		 if( $("#chkMGender"+id).is(":checked") ) wmsGender.push("m");
		 if( $("#chkWGender"+id).is(":checked") ) wmsGender.push("w");
		 $("#sage"+id).val() ? wmsSage = $("#sage"+id).val() : wmsSage = 0;
		 $("#eage"+id).val() ? wmsEage = $("#eage"+id).val() : wmsEage = 999;
		 $("#smember"+id).val() ? wmsSmember = $("#smember"+id).val() : wmsSmember = 0;
		 $("#emember"+id).val() ? wmsEmember = $("#emember"+id).val() : wmsEmember = 999;
	...
	var area = "";
	// atype
	if( features.length > 0 && atype == "user" ) {
		area = mapObj.getWktFromGeometry( features[0].getGeometry(), "EPSG:4326" );
	}
	...
	//연령계산의 기준년 가져오기
	 ...
	 var datas = {
	    ...
	    params  : {
		     ...
		     wmsYear: wmsYear,
		     wmsGender: wmsGender.join("_"),
		     wmsSage: wmsSage,
		     wmsEage: wmsEage,
		     wmsSmember: wmsSmember,
		     wmsEmember: wmsEmember
	 	},
