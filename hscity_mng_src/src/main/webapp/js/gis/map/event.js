...
function clkMnItem(num, flag, menu){
	...
	if(chkTsAnals(flag)){
		...
		searchJspPage("/gis/search.do", {flag:flag, menu:menu}, false, "gis-lnb"); //gisPopup side search 호출
		searchJspPage("/gis/result.do", {flag:flag, menu:menu}, false, "gis-result");
...
function clickRadiusSave(mapType){
	...
	var featureWkt = mapObj.mapCustom.getUserArea();
	if(featureWkt != null && featureWkt != ''){
		if(name != ''){
			$.ajax({
				type:"POST",
				url:"/gis/comm/addBmk.do",		
				dataType:'json',
				data:{geom:featureWkt,
	...
	if(flag == 'create'){
		...
		if(gbn == 'layer'){
			...
			if(seq != undefined && seq != null && seq != ''){				
				mapObj.mapCustom.userBmkFeature();
			}else{
				openInfoPopup(true,"선택된 사용자 지정영역이 없습니다.");
			}
		}else{
			mapObj.activeControl('ds', gbn);
		}
	}else if(flag == 'clear'){
