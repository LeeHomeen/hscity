...
<script type="text/javascript">
    ...
    $(document).ready(function(e){
        ...
        $('#btnList').on('click', function(){
            $searchVO.attr('action', '<c:url value="/admin/board/dataManegement/list.do"/>');
            $searchVO.submit();
        });
            ...
            if(validateBoardVO($boardVO[0])) {
                <c:if test="${registerFlag == '등록'}">
                    if(confirm("<spring:message code='common.regist.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/dataManegement/add.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
                ...
                <c:if test="${registerFlag == '수정'}">
                    if(confirm("<spring:message code='common.update.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/dataManegement/update.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
            }
        ...
        (function(){
            if(${!empty count} ) {
                if(${count > 0}) {
                    ...
                    $boardVO.attr('action', '<c:url value="/admin/board/dataManegement/view.do"/>');
                    $boardVO.submit();
                } else {
...
<div id="wrap">
    ...
    <div id="container">
        <%-- left 메뉴 --%>
        <jsp:include page="/com/leftMenu.do?type=board&menu=dataManegement"/>
        <div class="sec-right">
            <!-- 상단 경로 및 페이지 타이틀 -->
            <div class="top-bar">
                <div class="path-wrap">
                    ...
                    <span class="now">데이터도움말(자료관리) 글쓰기</span>
                </div>
                <div class="clearfix">
                    <p class="tit-page">데이터도움말(자료관리) 글쓰기</p>
                </div>
            </div>
            <!-- //상단 경로 및 페이지 타이틀 -->
            ...
            <div class="cont">
                ...
                <form id="boardVO" name="boardVO" method="post" enctype="multipart/form-data">
                    ...
                    <table class="tbl sr1 st1">
                        <caption class="hide">데이터도움말(자료관리) 글쓰기 표</caption>
                        <colgroup>
                            <col style="width:20%"/>
