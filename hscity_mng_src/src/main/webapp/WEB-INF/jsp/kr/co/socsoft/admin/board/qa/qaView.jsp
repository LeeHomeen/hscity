...
<script type="text/javascript">
    ...
    $(document).ready(function(e){
        ...
        $('#btnList').on('click', function(){
            $boardVO.attr('action', '<c:url value="/admin/board/qa/list.do"/>');
            $boardVO.submit();
        });

        // 수정
        $('#btnEdit').on('click', function(){
            $boardVO.attr('action', '<c:url value="/admin/board/qa/updateView.do"/>');
            $boardVO.submit();
        });
        ...
        $('#btnDelete').on('click', function(){
            if(confirm('<spring:message code="common.delete.msg"/>')) {
                $boardVO.attr('action', '<c:url value="/admin/board/qa/delete.do"/>');
                $boardVO.submit();
            }
        });
        ...
        $('#btnAnswerAdd').on('click', function(){
           ...
           if("${registerFlag}" == '답변등록'){ 
        	   if(confirm("<spring:message code='common.regist.msg'/>")) {
                   $($answerVO).find('input[name="seq"]').val('${result.seq}');
                   $answerVO.attr('action', '<c:url value="/admin/board/qa/add.do"/>');
                   $answerVO.submit();
               }
           }else if("${registerFlag}" == '답변수정'){
        	   if(confirm("<spring:message code='common.update.msg'/>")) {
                   $answerVO.attr('action', '<c:url value="/admin/board/qa/answerUpdate.do"/>');
                   $answerVO.submit();
               }
           }
...
<div id="wrap">
    ...
    <div id="container">
        <%-- left 메뉴 --%>
        <jsp:include page="/com/leftMenu.do?type=board&menu=qa"/>
        <div class="sec-right">
            <!-- 상단 경로 및 페이지 타이틀 -->
            <div class="top-bar">
                <div class="path-wrap">
                    ...
                    <span class="path">Q&A</span>&nbsp;&nbsp;&gt;
                    <span class="now">Q&A 상세보기</span>
                </div>
                <div class="clearfix">
                    <p class="tit-page">Q&A 상세보기</p>
                </div>
            </div>
            <!-- //상단 경로 및 페이지 타이틀 -->
            ...
            <div class="cont">
                ...
                <div style="margin-top: 30px;">
                    ...
                    <form id="answerVO" name="answerVO" method="post" enctype="multipart/form-data">
                        <input type="hidden" name="seq" value="${result.answers[0].seq}"/>
                        <input type="hidden" name="upperQnaSeq" value="${result.answers[0].upperQnaSeq}"/>
                        <input type="hidden" name="attFileMngSeq" value="${result.answers[0].attFileMngSeq}"/>
                        ...
                        <table class="tbl sr1 st1">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="align-l">
                                        <textarea id='contents' name='contents' cols="107" rows="30">${result.answers[0].contents}</textarea>
                                    </td>
                                </tr>
