...
<script type="text/javascript">
    ...
    $(document).ready(function(e){
        ...
        $('#btnList').on('click', function(){
            $searchVO.attr('action', '<c:url value="/admin/board/leehomeen/list.do"/>');
            $searchVO.submit();
        });
            ...
            if(validateBoardVO($boardVO[0])) {
                <c:if test="${registerFlag == '등록'}">
                    if(confirm("<spring:message code='common.regist.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/leehomeen/add.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
                ...
                <c:if test="${registerFlag == '수정'}">
                    if(confirm("<spring:message code='common.update.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/leehomeen/update.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
            }
        ...
        (function(){
            if(${!empty count} ) {
                if(${count > 0}) {
                    ...
                    $boardVO.attr('action', '<c:url value="/admin/board/leehomeen/view.do"/>');
                    $boardVO.submit();
                } else {
...
<div id="wrap">
    ...
    <div id="container">
        <%-- left 메뉴 --%>
        <jsp:include page="/com/leftMenu.do?type=board&menu=leehomeen"/>
        <div class="sec-right">
            <!-- 상단 경로 및 페이지 타이틀 -->
            <div class="top-bar">
                <div class="path-wrap">
