...
<script type="text/javascript">
    ...
    $(document).ready(function(e){
        ...
        $('#btnList').on('click', function(){
            $searchVO.attr('action', '<c:url value="/admin/board/qa/list.do"/>');
            $searchVO.submit();
        });
            ...
            if(validateBoardVO($boardVO[0])) {
                <c:if test="${registerFlag == '등록'}">
                    if(confirm("<spring:message code='common.regist.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/qa/add.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
                ...
                <c:if test="${registerFlag == '수정'}">
                    if(confirm("<spring:message code='common.update.msg'/>")) {
                        $boardVO.attr('action', '<c:url value="/admin/board/qa/update.do"/>');
                        $boardVO.submit();
                    }
                </c:if>
            }
        ...
        (function(){
            if(${!empty count} ) {
                if(${count > 0}) {
                    ...
                    $boardVO.attr('action', '<c:url value="/admin/board/qa/view.do"/>');
                    $boardVO.submit();
                } else {
        ...
        <div class="sec-right">
            ...
            <div class="top-bar">
                <div class="path-wrap">
                    ...
                    <span class="now">Q&A 글쓰기</span>
                </div>
                <div class="clearfix">
                    <p class="tit-page">Q&A 글쓰기</p>
                </div>
            </div>
            <!-- //상단 경로 및 페이지 타이틀 -->
            ...
            <div class="cont">
                ...
                <form id="boardVO" name="boardVO" method="post" enctype="multipart/form-data">
                    ...
                    <table class="tbl sr1 st1">
                        <caption class="hide">Q&A 글쓰기 표</caption>
                        <colgroup>
                            <col style="width:20%"/>
