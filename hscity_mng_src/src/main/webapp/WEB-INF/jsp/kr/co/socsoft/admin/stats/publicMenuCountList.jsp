...
$(document).ready(function(){
	...
	$.ajax({
	    ...
	    data: {categoryType: "pub", upperCategoryCd: ""},
	...
	$('#category1depth').on('change', function(event) {
		...
		$.ajax({
		    ...
		    dataType: 'json',
		    headers : { 'Content-Type' : 'application/json; charset=UTF-8'},
		    data: {categoryType: "pub", upperCategoryCd: $('#category1depth').val()},
		    success: function(response) {
	...
	$('#category2depth').on('change', function(event) {
		$.ajax({
		    ...
		    dataType: 'json',
		    headers : { 'Content-Type' : 'application/json; charset=UTF-8'},
		    data: {categoryType: "pub", upperCategoryCd: $('#category2depth').val()},
		    success: function(response) {
    ...
    $('#btnExcel').on('click', function(){
        if(confirm('데이터를 엑셀로 내려받으시겠습니까?\n데이터 건수가 많을 경우 오래 걸릴 수 있습니다.')) {
            ...
            CM.downloadFileWithIFrame($('body'), '<c:url value="/stats/userMenu/getPublicMenuCountExcel.do"/>', $('#searchVO').serializeObject());
        }
    });
    ...
    function initGrid($grid) {
        ...
        colModels[colIndex++]={name: "connIp", index: "conn_ip", comments: "접속IP", width: 80, align: "center", hidden: false};
        ...
        $grid.jqGrid({
            url: '<c:url value="/stats/userMenu/getPublicMenuCountList.do"/>',
            datatype: "json",
            mtype: 'get',
            postData: $("form[name='searchVO']").serializeObject(),
        ...
        <div class="sec-right">
            ...
            <div class="top-bar">
                <div class="path-wrap">
                    ...
                    <span class="now">대시민포털 메뉴접속통계</span>
                </div>
                <div class="clearfix">
                    <p class="tit-page">대시민포털 메뉴접속통계</p>
                </div>
            </div>
            <!-- //상단 경로 및 페이지 타이틀 -->
